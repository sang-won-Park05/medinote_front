...
export default function MedInfoTab() {
  ...
  useEffect(() => {
    const fetchMeds = async () => {
      try {
        ...
        const mapped = data.map(mapDrugToMedication);
        useHealthDataStore.setState({ medications: mapped });
      } catch (err) {
  ...
  const filtered = medications.filter((med) => filter === "all" || filter === med.type);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        ...
        {filtered.length === 0 && (
          <p className="text-sm text-gray-400 text-center p-4">추가된 복약 정보가 없습니다.</p>
        )}
      </div>
