...
export default function DashboardPage() {
  ...
  const today = kstYmd();
  const todays = useMemo(() => schedules.filter((s) => s.date === today).sort((a,b)=>a.time.localeCompare(b.time)), [schedules, today]);
  ...
  const handleMedCheck = (id: string) => setMedCheckState((p) => ({ ...p, [id]: !p[id] }));
  const totalMeds = medications.length;
  const checkedMeds = Object.values(medCheckState).filter(Boolean).length;
  const progressPercent = totalMeds > 0 ? (checkedMeds / totalMeds) * 100 : 0;
    ...
    <div className="flex flex-col p-4 pb-16 space-y-6">
      ...
      <section className="w-full bg-mint/10 rounded-lg shadow-sm overflow-hidden relative pt-6 pb-10">
        ...
        >
          ...
          <div className="w-full flex-shrink-0 px-6 min-h-[120px] flex flex-col justify-center">
            <h2 className="text-2xl font-bold text-dark-gray mb-1">
              <span className="text-mint">{userName}</span>님, 안녕하세요!
            </h2>
          ...
          <div className="w-full flex-shrink-0 px-6 min-h-[120px]">
            <div className="bg-white p-4 rounded-lg shadow-sm">
              <div className="flex justify-between items-center mb-2">
                ...
                <Link to="/schedule" className="text-xs text-mint hover:underline flex items-center">
                  전체보기 <HiOutlineArrowRight className="ml-1" />
                </Link>
              </div>
              {todays.length === 0 ? (
                <p className="text-sm text-gray-400">아직 오늘의 일정이 없습니다.</p>
              ) : (
                <ul className="space-y-1">
                  {todays.slice(0,3).map((s) => (
                    <li key={s.id} className="text-sm text-gray-700"><span className="font-semibold mr-2">{s.time}</span>{s.title}</li>
                  ))}
                  {todays.length > 3 && (
                    <li className="text-xs text-gray-400">+ {todays.length - 3}개 더 있음</li>
                  )}
                </ul>
              )}
            </div>
        ...
        <div className="absolute bottom-4 left-0 right-0 flex justify-center gap-2">
          <button 
            ...
            className={`w-2 h-2 rounded-full transition-all ${currentSlide === 0 ? 'bg-mint' : 'bg-mint/30'}`}
            aria-label="메인 배너 보기"
          />
          <button 
            onClick={() => setCurrentSlide(1)}
            className={`w-2 h-2 rounded-full transition-all ${currentSlide === 1 ? 'bg-mint' : 'bg-mint/30'}`}
            aria-label="오늘의 일정 보기"
          />
        </div>
      </section>
      ...
      <section className="w-full bg-white rounded-lg shadow-lg p-5">
        <div className="flex justify-between items-end mb-3">
          <h3 className="text-lg font-bold text-dark-gray">오늘의 복약 관리</h3>
        ...
        <div className="space-y-3 max-h-[220px] overflow-y-auto pr-1 custom-scrollbar">
          {medications.length > 0 ? (
            ...
            <div className="text-center py-6 bg-gray-50 rounded-lg border border-dashed border-gray-200">
              ...
              <Link to="/health-info" className="text-sm text-mint font-bold hover:underline mt-1 inline-block">
                + 약 추가하러 가기
              </Link>
            </div>
          )}
...
type MedCheckProps = { time: string; name: string; isChecked: boolean; onCheck: () => void };
  ...
  return (
    <label className="flex items-center p-3 bg-gray-50 rounded-lg cursor-pointer hover:bg-gray-100 border border-transparent hover:border-mint/30 transition-all">
      ...
      <div className="ml-3 flex-1">
        <p className={`text-sm font-bold mb-0.5 ${isChecked ? 'text-gray-400 line-through' : 'text-dark-gray'}`}>
          {name}
        </p>
        <p className={`text-xs ${isChecked ? 'text-gray-400 line-through' : 'text-gray-500'}`}>
          {time}
        </p>
      </div>
      {isChecked && <HiOutlineCheckCircle className="text-mint text-xl animate-bounce-short" />}
    </label>
  );
}
