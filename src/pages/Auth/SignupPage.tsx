  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-[#F8FBFB] p-4">
      ...
      <div className="w-full max-w-sm bg-white rounded-lg shadow-lg p-8">
        {step === 'terms' ? (
          <div>
            <h2 className="text-lg font-bold text-dark-gray mb-4">약관 동의</h2>
            ...
            <div className="space-y-3 max-h-72 overflow-auto pr-1">
              <TermsItem
                ...
                onChange={(v) => {
                  setAgreeSensitive(v);
                  setAgreeAll(v && agreeDisclaimer && agreeAccuracy && agreeBackup);
                }}
                content={
                  <>
                    <p>
                      본 서비스는 사용자의 건강 관리 및 맞춤형 서비스 제공을 위하여, 질병 정보,
                      복용 약 정보, 알러지 정보, 진료 기록 등 「개인정보 보호법」상 ‘민감정보’에
                      해당하는 항목을 수집·이용합니다.
                    </p>
                    <p className="mt-2">
                      회원가입을 진행함으로써 귀하는 상기 민감정보의 수집, 이용, 및 처리를
                      허용하는 것에 명시적으로 동의한 것으로 간주됩니다.
                    </p>
                  </>
                }
              />
              <TermsItem
                ...
                onChange={(v) => {
                  ...
                  setAgreeAll(v && agreeSensitive && agreeAccuracy && agreeBackup);
                }}
                content={
                  <>
                    <p>
                      본 서비스에서 제공하는 AI 기반 답변, 음성 요약, 건강분석 리포트 및 기타
                      정보는 사용자의 편의를 위한 참고 자료일 뿐이며, 의사·약사 등 전문 의료인의
                      진단, 처방, 또는 의학적 조언을 대체하지 않습니다.
                    </p>
                    <p className="mt-2">
                      건강상 이상 또는 질환이 의심되는 경우, 반드시 전문 의료기관을 방문하여 의료
                      전문가와 상담하시기 바랍니다.
                    </p>
                    <p className="mt-2">
                      본 서비스는 의료행위를 제공하지 않으며, 사용자가 AI 답변을 신뢰함으로써
                      발생하는 어떠한 결과에 대해서도 법적 책임을 부담하지 않습니다.
                    </p>
                  </>
                }
              />
              <TermsItem
                ...
                onChange={(v) => {
                  setAgreeAccuracy(v);
                  setAgreeAll(v && agreeSensitive && agreeDisclaimer && agreeBackup);
                }}
                content={
                  <>
                    <p>
                      본 서비스에 입력되는 건강정보(질환, 증상, 복용량, 약품명 등)는 사용자가 직접
                      입력한 내용을 기반으로 합니다.
                    </p>
                    <p className="mt-2">
                      사용자가 부정확하거나 오래된 정보를 입력함으로써 발생하는 결과, 오류, 또는
                      불이익에 대해서는 전적으로 사용자 본인에게 책임이 있습니다.
                    </p>
                    <p className="mt-2">
                      회사는 사용자가 입력한 데이터의 정확성, 완전성, 최신성에 대해 보증하지
                      않습니다.
                    </p>
                  </>
                }
              ...
              <TermsItem
                ...
                onChange={(v) => {
                  setAgreeBackup(v);
                  setAgreeAll(v && agreeSensitive && agreeDisclaimer && agreeAccuracy);
                }}
                content={
                  <>
                    <p>
                      회사는 사용자의 데이터가 안전하게 저장되도록 합리적인 기술적·관리적 조치를
                      취합니다.
                    </p>
                    <p className="mt-2">
                      그러나 서버 오류, 시스템 장애, 또는 예기치 못한 기술적 문제로 인해 데이터가
                      유실될 가능성이 있습니다.
                    </p>
                    <p className="mt-2">본 서비스는 이를 대비하여 PDF 내보내기 기능을 제공합니다.</p>
                    <p className="mt-2">
                      따라서 사용자는 중요한 건강 정보를 주기적으로 직접 내보내시기를 권장합니다.
                    </p>
                    <p className="mt-2">
                      회사는 고의 또는 중대한 과실이 없는 한, 데이터 유실로 인한 손해에 대해 책임을
                      부담하지 않습니다.
                    </p>
                  </>
                }
            ...
            <div className="text-center mt-4">
              <Link to="/login" className="text-sm text-gray-500 hover:text-mint">
                로그인으로 돌아가기
              </Link>
            </div>
          </div>
      ...
      <p className="text-center text-gray-400 text-xs mt-8">© 2025 메디노트</p>
...
function SignupForm({ onBack }: { onBack: () => void }) {
  ...
  const validateEmail = (v: string) => {
    ...
    if (!/.+@.+\.com$/i.test(v)) return '유효한 이메일 형식이 아닙니다';
    return undefined;
  };
  ...
  const handleBlur = (key: keyof typeof errors) => {
    ...
    if (key === 'password') setFieldError('password', validatePassword(password));
    if (key === 'confirm') setFieldError('confirm', validateConfirm(confirm, password));
  ...
  const onSubmit = (e: React.FormEvent) => {
    ...
    setSubmitting(false);
    const ok = Object.values(nextErrors).every((m) => !m);
    if (!ok) return;

    alert('회원가입이 완료되었습니다! 가입하신 이메일로 인증 메일을 발송했습니다.');
  ...
  return (
    <form onSubmit={onSubmit} noValidate>
      ...
      {/* 비밀번호 확인 */}
      <div className="mb-4">
        <label className="block text-sm font-bold text-dark-gray mb-2" htmlFor="confirm">
          비밀번호 확인
        </label>
        <div className="relative">
          <HiOutlineLockClosed className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
      ...
      {/* 이름 */}
      <div className="mb-6">
        <label className="block text-sm font-bold text-dark-gray mb-2" htmlFor="name">
          이름
        </label>
        <div className="relative">
    ...
    <div className="border rounded-lg p-3">
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1">
          ...
          {open && <div className="text-xs text-gray-500 mt-2 leading-relaxed">{content}</div>}
        </div>
      </div>
    </div>
  );
